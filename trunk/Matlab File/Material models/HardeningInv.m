function varargout = HardeningInv(action,MatData,stress)
%HARDENINGINV hardening material (one-dimensional rate-independent plasticity
% model with combined isotropic and kinematic hardening) for force method
% varargout = HardeningInv(action,MatData,strain)
%
...
F    = MatData.F;       % initial elastic modulus
epsy = MatData.epsy;    % yield strain
Hkin = MatData.Hkin;    % kinematic hardening flexibility
   ...
   case 'setTrialStress'
      ...
      if (abs(stressT(tag)-stress) > eps)
         ...
         % plastic step -> return mapping
            ...
            % update back strain
            strainBackT(tag) = strainBackC(tag) + dGamma*Hkin*signXi;
            
            % set trial tangent
            tangentT(tag) = F*Hkin/(F+Hkin);
         end
      end
