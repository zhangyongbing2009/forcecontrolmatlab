...
Z = zeros(1,length(Acc));

for nn = 2:length(Acc)
    [Q_0,Uf_0,Ufdot_0,Ufdotdot_0,Z_0] = NLForceEquations2(Bx,Bi,Uf(:,nn-1),Ufdot(:,nn-1),Ufdotdot(:,nn-1),m1,m2,c1,c2,dt,Acc(nn),Q(3,nn));
    Z(nn) = Z_0;
    Q(:,nn) = Q_0;
    Uf(:,nn) = Uf_0;
    Ufdot(:,nn) = Ufdot_0;
    ...
    if Z(nn) > Tol
        errordlg(['Analysis stopped at step ' num2str(nn) ])
        break;
    end
end
