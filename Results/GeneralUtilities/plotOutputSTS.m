...
for i=1:length(dataID)
   ...
   if strcmp(desc(dataID(i),7:15),'Force cmd')
      forceCmdID = [forceCmdID,dataID(i)];
   end
   if strcmp(desc(dataID(i),7:15),'Force scr')
      forceScrID = [forceScrID,dataID(i)];
   end
...
forceCmd = data(:,forceCmdID);
forceScr = data(:,forceScrID);
...
%==========================================================================
% command and measured forces
CreateWindow('cen',0.80*SS(4)/3*4,0.80*SS(4));
plot(time,forceScr,'-b');
hold('on');
plot(time,forceFbk,'-r');
grid('on');
xlabel('Time [sec]');
ylabel('Force [kip]');
title(['Forces from STS: Actuator ',num2str(actID)]);
legend('commFrc','measFrc');
%==========================================================================
% fft of error between command and measured forces
CreateWindow('cen',0.80*SS(4)/3*4,0.80*SS(4));
error = forceFbk - forceScr;
